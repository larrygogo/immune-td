...
export default class XMLHttpRequest extends EventTarget {
    ...
    send(data = '') {
        if (this.readyState !== XMLHttpRequest.OPENED) {
            ...
            // 修正：blob 也是 binary 数据，不能 utf8 解码（PNG 等会损坏）
            // 默认让 wx.readFile 返回 ArrayBuffer，仅 text/json 用 utf8
            if (responseType === 'arraybuffer' || responseType === 'blob') {
                // binary：不设 encoding，wx 返回 ArrayBuffer
            } else if (responseType === 'json' || responseType === 'text' || responseType === '') {
                encoding = 'utf8'
            } else {
                encoding = 'utf8'
            }
